...
mutators <- c("doImplosionMutation", "doClusterMutation", "doRotationMutation", "doLinearProjectionMutation")
dirs <- c("implosion", "cluster", "rotation", "linearprojection")
n_instances <- 4L
output_dir <- "."
...
for(idx in seq_along(mutators)) {
  ...
  for(i in 0:(n_instances - 1)) {
    ...
    x = build(n = 800L, iters = 2L, collection = collection, upper = 1000000, bound.handling = "uniform")
