...
for(idx in seq_along(clusters)) {
  ...
  for(i in 0:(n_instances - 1)) {
    sigmas = lapply(1:cluster, function(x) diag(runif(1, min = 2000000000, max = 5000000000), nrow = 2, ncol = 2))
    x = generateClusteredNetwork(n.cluster = cluster, n.points = 800L, lower = 0, upper = 1000000, sigmas=sigmas)
